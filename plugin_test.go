...
func TestOpenAPISkipEmpty(t *testing.T) {
	emptyTest := OpenAPITest{name: "No annotations example", path: "examples/tests/no_annotations/", protofile: "message.proto"}
	t.Run(emptyTest.name, func(t *testing.T) {
		...
		cmd := []string{
			...
			"./",
			"-I", "./examples",
			path.Join(emptyTest.path, emptyTest.protofile),
			"--openapi_out=output_mode=source_relative,skip_empty=true:.",
		}
		...
		// Check that the generated OpenAPI spec does not exists.
		_, err = os.Stat(path.Join(emptyTest.path, "message.openapi.yaml"))
...
// 				out, err := exec.Command("diff", TEMP_FILE, fixture).Output()
// 				if err != nil {
// 					fmt.Println("Diff failed:", err, string(out))
